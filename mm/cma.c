...
static struct rb_legacy_root cma_chunks_by_start;
...
static struct cma_chunk *__must_check __cma_chunk_find(dma_addr_t addr)
{
	...
	for (n = cma_chunks_by_start.rb_legacy_node; n; ) {
...
{
	struct rb_legacy_node **new, *parent = NULL;
	typeof(chunk->start) addr = chunk->start;

	for (new = &cma_chunks_by_start.rb_legacy_node; *new; ) {
		struct cma_chunk *c =
	...
	rb_legacy_link_node(&chunk->by_start, parent, new);
	rb_legacy_insert_color(&chunk->by_start, &cma_chunks_by_start);
...
{
	rb_legacy_erase(&chunk->by_start, &cma_chunks_by_start);
	   ...
	   size_t size, dma_addr_t alignment)
{
	...
	/*printk("allocate %p/%p from one of %s\n",*/
