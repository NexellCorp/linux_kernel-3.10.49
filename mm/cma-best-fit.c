...
struct cma_bf_item {
	struct cma_chunk ch;
	struct rb_legacy_node by_size;
};

struct cma_bf_private {
	struct rb_legacy_root by_start_root;
	struct rb_legacy_root by_size_root;
};
...
int cma_bf_init(struct cma_region *reg)
{
	...
	rb_legacy_root_init(&prv->by_start_root, &item->ch.by_start);
	rb_legacy_root_init(&prv->by_size_root, &item->by_size);
...
void cma_bf_cleanup(struct cma_region *reg)
{
	...
	struct cma_bf_item *item =
		rb_legacy_entry(prv->by_size_root.rb_legacy_node,
...
{
	struct cma_bf_private *prv = reg->private_data;
	struct rb_legacy_node *node = prv->by_size_root.rb_legacy_node;
	struct cma_bf_item *item = NULL;

	/* First find hole that is large enough */
	while (node) {
		struct cma_bf_item *i =
			rb_legacy_entry(node, struct cma_bf_item, by_size);
	...
	for (;;) {
		...
		node = rb_legacy_next(node);
		if (!node)
			return NULL;

		item  = rb_legacy_entry(node, struct cma_bf_item, by_size);
	}
}
...
static void __cma_bf_hole_insert_by_size(struct cma_bf_item *item)
{
	...
	struct rb_legacy_node **link = &prv->by_size_root.rb_legacy_node, *parent = NULL;
	const typeof(item->ch.size) value = item->ch.size;

	while (*link) {
		struct cma_bf_item *i;
		parent = *link;
		i = rb_legacy_entry(parent, struct cma_bf_item, by_size);
		link = value <= i->ch.size
			? &parent->rb_left
			: &parent->rb_right;
	}

	rb_legacy_link_node(&item->by_size, parent, link);
	rb_legacy_insert_color(&item->by_size, &prv->by_size_root);
}

static void __cma_bf_hole_erase_by_size(struct cma_bf_item *item)
{
	struct cma_bf_private *prv = item->ch.reg->private_data;
	rb_legacy_erase(&item->by_size, &prv->by_size_root);
}
...
{
	struct cma_bf_private *prv = item->ch.reg->private_data;
	struct rb_legacy_node **link = &prv->by_start_root.rb_legacy_node, *parent = NULL;
	const typeof(item->ch.start) value = item->ch.start;

	while (*link) {
		struct cma_bf_item *i;
		parent = *link;
		i = rb_legacy_entry(parent, struct cma_bf_item, ch.by_start);
	...
	rb_legacy_link_node(&item->ch.by_start, parent, link);
	rb_legacy_insert_color(&item->ch.by_start, &prv->by_start_root);
	return 0;
}
...
{
	struct cma_bf_private *prv = item->ch.reg->private_data;
	rb_legacy_erase(&item->ch.by_start, &prv->by_start_root);
}
...
static void __cma_bf_hole_merge_maybe(struct cma_bf_item *item)
{
	...
	node = rb_legacy_prev(&item->ch.by_start);
	if (unlikely(!node))
		goto next;
	prev = rb_legacy_entry(node, struct cma_bf_item, ch.by_start);
...
next:
		...
		node = rb_legacy_next(&item->ch.by_start);
		if (unlikely(!node))
			break;
		prev = item;
		item = rb_legacy_entry(node, struct cma_bf_item, ch.by_start);
	}
}
