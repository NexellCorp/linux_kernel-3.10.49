...
end_flow:
	...
	if ( info->soca->chg_status == POWER_SUPPLY_STATUS_CHARGING ) {
		err = nxe2000_read(info->dev->parent, NXE2000_REG_TIMSET, &val);
		if (err < 0)
			dev_err(info->dev,
			"Error in read NXE2000_REG_TIMSET %d\n", err);
		/* to check bit 0-1 */
		val2 = val & 0x03;

		if (val2 == 0x02){
			/* set rapid timer 240 -> 300 */
			err = nxe2000_set_bits(info->dev->parent, NXE2000_REG_TIMSET, 0x03);
			if (err < 0) {
				dev_err(info->dev, "Error in writing the control register\n");
			}
		} else {
			/* set rapid timer 300 -> 240 */
			err = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_TIMSET, 0x01);
			err = nxe2000_set_bits(info->dev->parent, NXE2000_REG_TIMSET, 0x02);
			if (err < 0) {
				dev_err(info->dev, "Error in writing the control register\n");
			}
		}
...
static __devinit int nxe2000_battery_probe(struct platform_device *pdev)
{
	...
	ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_INTEN, CHG_INT | ADC_INT);
	if (ret)
		goto out;
	...
	nxe2000_set_bits(info->dev->parent, NXE2000_REG_INTEN, CHG_INT | ADC_INT);
...
//	nxe2000_power_suspend_status    = 1;
	...
	nxe2000_clr_bits(dev->parent, NXE2000_REG_INTEN, CHG_INT);
#endif
...
#ifdef ENABLE_MASKING_INTERRUPT_IN_SLEEP
	nxe2000_set_bits(dev->parent, NXE2000_REG_INTEN, CHG_INT);
#endif
	ret = check_jeita_status(info, &is_jeita_updated);
	if (ret < 0) {
		dev_err(info->dev, "Error in updating JEITA %d\n", ret);
