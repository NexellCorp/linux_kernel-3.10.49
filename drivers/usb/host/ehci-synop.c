...
static	struct nxp_ehci_platdata *pdata;
static int nxp_ehci_probe(struct platform_device *pdev)
{
	...
	pdata->phy_init = nxp_usb_phy_init;
	pdata->phy_exit = nxp_usb_phy_exit;
	pdata->hsic_phy_pwr_on = nxp_hsic_phy_pwr_on;
	...
	pdev->dev.dma_mask = &nxp_ehci_dmamask;
...
fail_hcd:
	if (nxp_ehci->phy)
		usb_put_phy(nxp_ehci->phy);
	kfree(nxp_ehci);
	return err;
}

static int nxp_ehci_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_ehci_match[] = { 
    { .compatible = "nexell,nxp-ehci" },
    {},
};
MODULE_DEVICE_TABLE(of, nxp_ehci_match);
#else
#define nxp_ehci_match NULL
#endif

static struct platform_driver nxp_ehci_driver = {
	.probe		= nxp_ehci_probe,
	.remove		= nxp_ehci_remove,
	.shutdown	= nxp_ehci_shutdown,
	.driver = {
		.name	= "nxp-ehci",
		.owner	= THIS_MODULE,
		.pm	= &nxp_ehci_pm_ops,
        .of_match_table = of_match_ptr(nxp_ehci_match),
	}
};
