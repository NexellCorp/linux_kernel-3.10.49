...
#define USB2_HOST_CLKGEN     IO_ADDRESS(PHYS_BASE_CLK_32)
...
int hsic_exthub_gpio = 0;
EXPORT_SYMBOL(hsic_exthub_gpio);
#endif
...
//		temp   |=  (3<<24);     /* Digital 3.3V */
	...
	else
	{
		...
		if (type == NXP_USB_PHY_HSIC) {
			of_property_read_u32(pdev->dev.of_node, "hsic_exthub_gpio", &hsic_exthub_gpio);
			if (hsic_exthub_gpio > 0) {
			    // GPIO Reset
				nxp_soc_gpio_set_io_dir(hsic_exthub_gpio, CTRUE);
			    nxp_soc_gpio_set_io_pull_enb(hsic_exthub_gpio, CTRUE);
			    nxp_soc_gpio_set_out_value(hsic_exthub_gpio, CTRUE);
				udelay( 10 );
			    nxp_soc_gpio_set_out_value(hsic_exthub_gpio, CFALSE);
			    udelay( 10 );
			    nxp_soc_gpio_set_out_value(hsic_exthub_gpio, CTRUE);
			} else
...
/*------------------------------------------------------------------------------
   * USB HSIC power control.
    */
#define SOC_PA_EHCI     PHYS_BASE_EHCI
