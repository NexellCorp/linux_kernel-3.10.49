...
static int nxp_rtc_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(IRQ_RTC, nxp_rtc_interrupt, IRQF_DISABLED,  "rtc 1hz", rtc);
	if (ret) {
...
static int nxp_rtc_remove(struct platform_device *pdev)
{
	struct rtc_device *rtc = platform_get_drvdata(pdev);

	pr_debug("%s\n", __func__);

	free_irq(IRQ_RTC, rtc);
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_rtc_dt_match[] = {
		{ .compatible = "nexell,nxp-rtc"},
		{},
};
MODULE_DEVICE_TABLE(of, nxp_rtc_dt_match);
#else
#define nxp_rtc_dt_match NULL
#endif

static struct platform_driver rtc_plat_driver = {
	.probe		= nxp_rtc_probe,
	.remove		= nxp_rtc_remove,
	.suspend	= nxp_rtc_suspend,
	.resume		= nxp_rtc_resume,
	.driver		= {
		.name	= "nxp-rtc",
		.owner	= THIS_MODULE,
		.of_match_table	= of_match_ptr(nxp_rtc_dt_match),		
	},
};
