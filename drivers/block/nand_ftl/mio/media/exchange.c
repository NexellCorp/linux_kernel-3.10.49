...
void EXCHANGE_init(void)
{
     ...
     **************************************************************************/
	if (nxp_nand.nxp_ftl_start_block & (_BLOCK_ALIGN_-1))
    {
		nxp_nand.nxp_ftl_start_block = ALIGN(nxp_nand.nxp_ftl_start_block, _BLOCK_ALIGN_);
	}

    Exchange.ewsftl_start_offset  = nxp_nand.nxp_ftl_start_block;
