...
static const char *in_pol_name;

static int nxp_ir_recv_probe(struct platform_device *pdev)
{
	struct nxp_rc_dev *nxp_dev;
	struct rc_dev *rcdev;
	const struct nxp_ppm_drv_data *pdata = NULL;
	struct clk *clk;
	int rc, in_pol_val = 0;
#ifdef CONFIG_PPM_SYSFS
	struct kobject * kobj;
	...
	if (pdev->dev.platform_data){
		...
		if (!of_property_read_string(pdev->dev.of_node, "in_pol", &in_pol_name)) {
			if (!strcmp(in_pol_name, "invert")) {
				pr_debug("#######invert####### %s\n", in_pol_name);
				in_pol_val = NX_PPM_INPUTPOL_INVERT;
			}
			else {
				pr_debug("#######bypass####### %s\n", in_pol_name);
				in_pol_val = NX_PPM_INPUTPOL_BYPASS;
			}
		} else
			printk("=========ERROR!!!!!!!!!!\n");
	}
...
//	NX_PPM_OpenModule(0);
	...
	if (pdev->dev.platform_data)
		...
		nxp_dev->input_polarity = in_pol_val;
	...
	nxp_dev->ppm_workqueue = create_singlethread_workqueue("nxp-ppm");
	if (!nxp_dev->ppm_workqueue) {
		rc = -ESRCH;
		goto exit_create_singlethread;
	}

	rc = request_irq(IRQ_PPM,
...
static int nxp_ir_recv_remove(struct platform_device *pdev)
{
	struct nxp_rc_dev *nxp_dev = platform_get_drvdata(pdev);

	free_irq(IRQ_PPM, nxp_dev);
	cancel_work_sync(&nxp_dev->ppm_event_work);
	destroy_workqueue(nxp_dev->ppm_workqueue);
	platform_set_drvdata(pdev, NULL);
...
#ifdef CONFIG_OF
static const struct of_device_id nxp_ir_dt_match[] = {
			{ .compatible = "nexell,nxp-ppm"},
					{},
};
MODULE_DEVICE_TABLE(of, nxp_ir_dt_match);
#else
#define nxp_ir_dt_match NULL
#endif

static struct platform_driver nxp_ir_recv_driver = {
	.probe  = nxp_ir_recv_probe,
	.remove = nxp_ir_recv_remove,
	.driver = {
		.name   = "nxp-ppm",
		.owner  = THIS_MODULE,
#ifdef CONFIG_PM
...
#endif
		.of_match_table	= of_match_ptr(nxp_ir_dt_match),
	},
};
