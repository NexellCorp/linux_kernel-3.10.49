...
#ifdef CONFIG_OF
static int ft5x0x_i2c_ts_probe_dt(struct device *dev, struct nxp_ts_cali_plat_data **ppdata)
{
	...
	if(of_property_read_u32(np, "mtouch-irq", &pdata->touch_irq)) {
		pr_err("incorrect irq number\n");
		return -EINVAL;
	}

	/* fill plat_data from device tree */
	of_property_read_u32(np, "touch_points", &pdata->touch_points);
...
#else
static int ft5x0x_i2c_ts_probe_dt(struct device *dev, struct nxp_ts_cali_plat_data **pdata)
{
	return -ENODEV;
}
#endif
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "failed to i2c functionality.\n");
		return -ENODEV;
	}
	...
	if (!pdata) {
		err = ft5x0x_i2c_ts_probe_dt(&client->dev, &pdata);
		if (err) {
			dev_err(&client->dev,
