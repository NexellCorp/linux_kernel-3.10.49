...
static int nxp_wdt_probe(struct platform_device *pdev)
{
	struct device *dev;
	unsigned int wtcon;
	int started = 0;
    ...
    base = ioremap(wdt_mem->start, resource_size(wdt_mem));
    if (!base) {
        printk(KERN_ERR "failed to ioremap memory resource for wdt\n");
        return -ENOMEM;
    }    
    DBG("resource %p (%lx..%lx)\n",
        (void*)wdt_mem, (long)wdt_mem->start, (long)wdt_mem->end);

	wdt_clock = clk_get(&pdev->dev, NULL);
	if (IS_ERR(wdt_clock)) {
	...
	ret = request_irq(wdt_irq, nxp_wdt_irq, 0, pdev->name, pdev);
	if (ret != 0) {
	...
	INIT_DEFERRABLE_WORK(&wdt_task_work, nxp_wdt_task_work);
...
static int nxp_wdt_remove(struct platform_device *dev)
{
	watchdog_unregister_device(&nxp_wdd);

	free_irq(wdt_irq, dev);
...
static struct platform_driver nxp_wdt_driver = {
	...
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "nxp-wdt",
		.of_match_table	= of_match_ptr(nxp_wdt_match),
	},
};
